...
class _PETScRHS(_data.Data):
    """
    A QuTiP data layer wrapper for PETSc distributed matrices.
    """
    def __init__(self, mat, shape, sys_size=None):
        super().__init__(shape)
        self.mat = mat
        self.sys_size = sys_size

    def copy(self):
        return _PETScRHS(self.mat, self.shape, self.sys_size)

def _petsc_to_csr(petsc_data):
    raise NotImplementedError("Conversion from _PETScRHS to CSR is not implemented.")

def _csr_to_petsc(csr_data):
    raise NotImplementedError("Conversion from CSR to _PETScRHS is not implemented.")

_data.to.add_conversions([
    (_PETScRHS, _PETScRHS, lambda x: x.copy(), 1),
    (_PETScRHS, _data.CSR, _csr_to_petsc, 100),
    (_data.CSR, _PETScRHS, _petsc_to_csr, 100),
])
_data.to.register_aliases(["petsc", "PETSc"], _PETScRHS)
_data.iszero.add_specialisations([
    (_PETScRHS, lambda x, *args, **kwargs: False)
])
...
class PETScHEOMSolver(HEOMSolver):
    ...
    def _calculate_rhs(self):
        ...
        petsc_data = _PETScRHS(mat, (dim, dim), sys_size=self._sup_shape)
        rhs_qobj = Qobj(petsc_data, dims=[[dim], [dim]], copy=False)
        
        return QobjEvo(rhs_qobj, compress=False)
    ...
    def steady_state(
        ...
        **kwargs
        ...
        n = self._sys_shape
        mat = self.rhs(0).data.mat.duplicate(copy=True)
        mat.setOption(PETSc.Mat.Option.NEW_NONZERO_ALLOCATION_ERR, False)
...
class IntegratorPETSc(Integrator):
    ...
    def _prepare(self):
        ...
        qobj = self.rhs_obj(0)
        if not isinstance(qobj.data, _PETScRHS):
            raise TypeError("IntegratorPETSc requires the QobjEvo to contain a _PETScRHS object.")
            
        self.mat = qobj.data.mat
        self.sys_size = qobj.data.sys_size
    ...
    def set_state(self, t, state0):
        ...
        elif len(state_np) == self.sys_size:
            start_idx = max(0, rstart)
            end_idx = min(self.sys_size, rend)
